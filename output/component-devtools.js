    ...
    if (trimmed.startsWith("[")) {
        try {
            ...
            if (Array.isArray(parsed)) {
                return ensureHeadless(parsed.filter((item) => typeof item === "string"));
            }
        }
        catch {
            // fall through
...
function ensureHeadless(args) {
    const hasHeadlessFlag = args.some((arg) => arg === "--headless" || arg.startsWith("--headless="));
    if (hasHeadlessFlag)
        return args;
    return [...args, "--headless"];
}
export function getChromeDevtoolsMcpConfig() {
    return {
...
function parseDiscoveryPayload(text) {
...
class ChromeDevtoolsMcpClient {
    ...
    static async connect() {
        ...
        instance.client = new Client({ name: "pixel-guard", version: "1.0.0" }, { capabilities: {} });
    ...
    async callTool(name, args = {}) {
        if (!this.client)
            throw new Error("chrome-devtools-mcp client is not connected");
        const result = await this.client.callTool({ name, arguments: args });
        return result;
    }
    async preparePage(pageUrl, viewport) {
        if (this.preparedPageUrl === pageUrl)
            return;
    ...
    async screenshotSelector(selector, filePath) {
...
function findSnapshotUid(snapshotText, marker) {
...
export async function discoverComponentsViaChromeDevtoolsMcp(pageUrl, viewport) {
    let client = null;
    try {
...
export async function screenshotComponentsViaChromeDevtoolsMcp(items, viewport) {
    if (items.length === 0)
        return [];
    let client = null;
    const results = [];
