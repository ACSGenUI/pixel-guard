...
**Goal:** Add two new pixel-guard MCP tools — `comparePageDiff` and `localizePageDiff` — that screenshot a live URL against its migrated counterpart at multiple viewports, pixel-diff them, cluster the differences into regions with cropped before/after/diff images and a browsable HTML report, and (on request) localize each region to the migrated page's DOM element(s) with selector/computed-style/outerHTML detail.

**Architecture:** All core logic lives as plain-JS, dependency-light Node scripts under `aem-visual-checker/tools/page-diff/` — copied into the target AEM project the same way `tools/visual-tests/` already is — runnable directly (`node tools/page-diff/compare-page-diff.js --mapping urls.csv`) or via `npm run test:page-diff:*`. Two thin Mastra workflows/tools in pixel-guard's own `src/` shell out to those same scripts via `execFile` and format the result for an MCP client, mirroring the existing `generateVisualTests` tool's relationship to `npm run test:visual:generate`.

**Tech Stack:** Node.js (`node:test`, `node:util.parseArgs`, `node:child_process`), `playwright` (already a target-project dependency), `pixelmatch` + `pngjs` (new target-project dependencies), Mastra (`@mastra/core`, `@mastra/mcp`), Zod, TypeScript (pixel-guard's own `src/`).
...
- New target-project npm dependencies (`pixelmatch`, `pngjs`) go through `aem-visual-checker/changes.js`'s `dependenciesToAdd`, exactly like `playwright`/`@playwright/test` already do — never hand-installed elsewhere.
- Pixel-guard's own repo needs `pixelmatch`/`pngjs` too, as **devDependencies** in the root `package.json`, purely so its own unit tests (which import these `aem-visual-checker/tools/page-diff/lib/*.js` modules directly) can run — this is separate from the `dependenciesToAdd` entry above.
...
**Interfaces:**
- Produces: `generateReportHtml(runSummary: RunSummary): string` — used by Task 8's `compare-page-diff.js` and Task 9's `localize-page-diff.js`. `RunSummary` shape (also the schema Task 11/12's Zod types must match):
...
- Produces: `runCompare({ mappingFile: string, targetDir: string }): Promise<{ runId: string, runDir: string, summary: RunSummary }>` — used by Task 9's `localize-page-diff.js` and Task 11's pixel-guard workflow. Also a CLI entrypoint printing `Run ID: <runId>` and `Report: <path>` to stdout.
...
1. In a target AEM EDS project that already has pixel-guard's `aemVisualTestInstall` run (so `playwright`, `pixelmatch`, `pngjs` are installed — see Tasks 10-11 for wiring this in), copy `aem-visual-checker/tools/page-diff/` into `tools/page-diff/` manually for now (the installer copy is wired up in Task 10).
...
## Task 10: Wire page-diff into the installer (npm scripts, dependencies, ignores)

**Files:**
- Modify: `aem-visual-checker/changes.js`

**Interfaces:**
- No new exports — `scripts`, `dependenciesToAdd`, `gitignoreLines`, `hlxignoreLines` already flow through `mergePackageJson`/`appendMissingLines` (existing `src/workflows/update-project-config.ts`, unchanged). Since `copyRequiredFiles` (`src/workflows/update-project-config.ts:58`) already does a recursive `cp` of the whole `tools/` source directory, adding files under `aem-visual-checker/tools/page-diff/` is automatically picked up by the existing `aemVisualTestInstall` flow with no code changes there.

- [ ] **Step 1: Add page-diff npm scripts, dependencies, and ignore entries**

In `aem-visual-checker/changes.js`, update:
...
export const hlxignoreLines = ["tools/visual-tests/*", "tools/page-diff/*"];
```

- [ ] **Step 2: Verify the file is still valid and existing tests still pass**

Run: `node -e "import('./aem-visual-checker/changes.js').then((m) => console.log(Object.keys(m)))"`
Expected: prints `[ 'scripts', 'dependenciesToAdd', 'devDependenciesToAdd', 'gitignoreLines', 'hlxignoreLines' ]` with no errors.

Run: `npm test`
Expected: PASS (existing `update-project-config.test.ts` tests exercise `mergePackageJson`/`appendMissingLines` generically, not `changes.js`'s literal content, so this is a smoke check that nothing broke).

- [ ] **Step 3: Commit**

```bash
git add aem-visual-checker/changes.js
git commit -m "feat: wire page-diff npm scripts/dependencies/ignores into the installer"
```

---

## Task 11: `comparePageDiff` Mastra workflow + MCP tool
...
- Produces: `runSummarySchema` (Zod schema matching Task 7's `RunSummary` shape) and `comparePageDiffWorkflow`, both exported from `src/workflows/compare-page-diff.ts` — the latter consumed by Task 12 (which imports `runSummarySchema` for its own output type) and `comparePageDiffTool` from `src/tools/compare-page-diff-tool.ts`, registered in Task 13.
...
Run: `npm test`
Expected: PASS (no new automated tests in this task — Mastra workflow wiring is exercised manually against a real target project in Task 14).
...
## Task 12: `localizePageDiff` Mastra workflow + MCP tool
...
**Interfaces:**
- Consumes: `runSummarySchema` from Task 11 (`src/workflows/compare-page-diff.ts`), `resolveProjectDir`, `serveReport`.
- Produces: `localizePageDiffWorkflow` and `localizePageDiffTool`, registered in Task 13.
...
## Task 13: Register the new tools and update docs
...
**Interfaces:**
- Consumes: `comparePageDiffTool` (Task 11), `localizePageDiffTool` (Task 12).

- [ ] **Step 1: Register both tools in `src/mcp-server.ts`**
...
const INSTRUCTIONS = `pixel-guard installs and drives a Playwright-based visual regression test suite for AEM Edge Delivery Services (EDS) projects. It renders every block variation from the target project's Sidekick Library at multiple viewports, screenshots them, and compares against committed baselines to catch unintended visual changes. It also includes page-diff: given a mapping of live URLs to their migrated counterparts, it screenshots and pixel-diffs both sides and can localize each visual difference to the migrated page's DOM element(s).
...
5. comparePageDiff -- given a CSV/JSON mapping file of { liveUrl, migratedUrl } pairs, screenshots and pixel-diffs both sides at every configured viewport, reporting which page regions differ with cropped before/after/diff images and a browsable HTML report.
6. localizePageDiff -- after comparePageDiff finds failing regions, finds the migrated page's overlapping DOM element(s) for each region and reports their selector, computed style, and outerHTML, so a fix can be reasoned about precisely instead of guessed from a diff image alone.
...
- [ ] **Step 2: Add a tools-table row and example prompts to `README.md`**

In the `## Tools` table in `README.md`, add two rows after `installVisualTestAutomation`:
...
git commit -m "feat: register comparePageDiff and localizePageDiff MCP tools"
```
...
- [ ] **Step 2: Fresh install into a real (or throwaway) AEM EDS project**

Using an AEM EDS project that doesn't yet have pixel-guard installed (or re-running install on one that does), call `aemVisualTestInstall` via the MCP server (or `npm run studio` and invoke it from Mastra Studio). Confirm:
- `tools/page-diff/` now exists in the target project (copied automatically by the existing `copyRequiredFiles` step, per Task 10's note).
- `package.json` in the target project now has `pixelmatch`, `pngjs` in `dependencies` and `test:page-diff:compare`/`test:page-diff:localize` in `scripts`.
- `node_modules/pixelmatch` and `node_modules/pngjs` exist after install (i.e. `npm install` picked up the new dependencies).
