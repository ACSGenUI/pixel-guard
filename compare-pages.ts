...
export type ComparePagesResult = {
  ...
  summary: string;
  artifacts: { source: string; destination: string; diff: string };
  createdAt: string;
  reportDir: string;
  reportBasePath: string;
};

function padImageToSize(img: PNG, targetWidth: number, targetHeight: number): PNG {
...
function comparePngBuffers(sourceImg: PNG, destImg: PNG) {
...
export async function comparePages({
  ...
  try {
    ...
    const { diff, diffPixels, totalPixels, diffPixelRatio, width, height } =
      comparePngBuffers(sourceImg, destImg);
