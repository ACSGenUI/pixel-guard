...
After comparePageDiff runs, review the page-diff report (full-page comparison, then block roll-up, then per-region detail) and give the user a complete summary: where the differences come from, the major fixes, and the quick fixes. Then fix broken blocks top-to-bottom in page order, NOT by severity: the roll-up is ordered top-to-bottom, so start at the top -- the header/nav if broken, then the topmost affected block -- and work straight down the page, using captureLiveBlock -> compareBlock for each one -- do NOT take your own screenshots of the live or migrated pages, since those tools already capture, locate, and diff each block. (captureLiveBlock and compareBlock ship inside tools/page-diff/, so re-run installPageDiff in the target project after upgrading pixel-guard to make them available.)
