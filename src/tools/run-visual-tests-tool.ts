...
export const runVisualTestsTool = createTool({
  id: 'runVisualTests',
  description: 'Runs the Playwright visual tests (all, or a single block when blockName is given).',
  ...
  execute: async ({ blockName, projectDir }) => {
    ...
    const text = output
      ? formatWorkflowResult('Run Visual Tests', [
      ...
      : `Run Visual Tests did not complete (status: ${result.status}).`;
