...
async function waitForServerReady(url: string, timeoutMs: number, pollIntervalMs: number): Promise<boolean> {
...
export async function startDevServer(cwd: string): Promise<{ started: boolean; message: string }> {
...
export function stopDevServer(): { stopped: boolean; message: string } {
  ...
  try {
    ...
    return {
      stopped: false,
      message: `Failed to stop the dev server: ${(error as Error).message}`,
    };
  } finally {
